...
def apply_special_reading_fixes(text):
    """単漢字・身体部位誤変換による誤読の修復（気象予報士➔し、肩甲骨➔けんこうこつ 等）"""
    if not text:
        return ""
    t = text
    # 1. OpenJTalk辞書で「カタカブトコツ」と誤読される肩甲骨・肩甲の正規化
    t = re.sub(r'肩甲骨', 'けんこうこつ', t)
    t = re.sub(r'肩甲', 'けんこう', t)

    # 2. 資格・役職・人名の「士（し）」が誤って「さむらい」に変換された場合の修復
