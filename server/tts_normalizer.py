...
IMPERIAL_ANCHOR_KEYWORDS = {
    ...
    "両陛下", "ご一家", "皇嗣", "皇嗣妃", "皇太子",
    "愛子さま", "佳子さま", "悠仁さま", "紀子さま", "雅子さま"
}

DEFAULT_IMPERIAL_PROPER_NOUNS = [
    # 尊称・敬称付き（名字なし）
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])文仁親王妃紀子(?:さま|様)?殿下', 'ふみひとしんのうひきこひでんか'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])文仁親王妃紀子(?:さま|様)', 'ふみひとしんのうひきこさま'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])文仁親王妃紀子', 'ふみひとしんのうひきこ'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])紀子妃殿下', 'きこひでんか'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])紀子妃(?:さま|様)', 'きこひさま'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])紀子妃', 'きこひ'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])紀子殿下', 'きこでんか'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])紀子(?:さま|様)', 'きこさま'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])親王妃', 'しんのうひ'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])妃殿下', 'ひでんか'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])悠仁(?:さま|様)', 'ひさひとさま'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])佳子(?:さま|様)', 'かこさま'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])愛子(?:さま|様)', 'あいこさま'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])秋篠宮(?:さま|様)', 'あきしののみやさま'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])秋篠宮ご夫妻', 'あきしののみやごふさい'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])秋篠宮家', 'あきしののみやけ'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])秋篠宮', 'あきしののみや'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])天皇陛下', 'てんのうへいか'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])皇后雅子(?:さま|様)', 'こうごうまさこさま'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])雅子皇后', 'まさここうごう'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])皇后陛下', 'こうごうへいか'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])皇后(?:さま|様)', 'こうごうさま'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])雅子(?:さま|様)', 'まさこさま'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])上皇陛下', 'じょうこうへいか'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])上皇(?:さま|様)', 'じょうこうさま'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])上皇后美智子(?:さま|様)', 'じょうこうごうみちこさま'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])上皇后(?:さま|様)', 'じょうこうごうさま'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])美智子(?:さま|様)', 'みちこさま'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])悠仁親王', 'ひさひとしんのう'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])愛子内親王', 'あいこないしんのう'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])佳子内親王', 'かこないしんのう'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])文仁親王', 'ふみひとしんのう'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])徳仁親王', 'なるひとしんのう'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])彬子(?:さま|様)', 'あきこさま'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])瑶子(?:さま|様)', 'ようこさま'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])承子(?:さま|様)', 'つぐこさま'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])常陸宮(?:さま|様)?', 'ひたちのみや'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])三笠宮(?:さま|様)?', 'みかさのみや'),
    (r'(?<![一-龥ぁ-んァ-ヶA-Za-z])高円宮(?:さま|様)?', 'たかまどのみや'),
]
...
def extract_imperial_pronunciations(text: str, title: str = "") -> dict:
    ...
    - 直前に漢字・かな・英字がある一般人（柏木由紀子、佐藤紀子、松本佳子、山田悠仁等）は 100% 除外・保護
    - 一般女性・男性名にもなりうる単独名（紀子さま、愛子さま、佳子さま等）は【皇室記事である場合のみ】登録
    """
    ...
    pron_map = {}
    # 1. 尊称付き皇族（直前に漢字・かな・英字なし）
    for pat, rep in DEFAULT_IMPERIAL_PROPER_NOUNS:
        # 一般女性名・男性名になりうる単独名（紀子さま、愛子さま、佳子さま等）は、皇室文脈の場合のみ登録
        is_solo_name_pat = any(n in pat for n in ("紀子(?:さま|様)", "愛子(?:さま|様)", "佳子(?:さま|様)", "悠仁(?:さま|様)", "雅子(?:さま|様)"))
        if is_solo_name_pat and not is_imperial:
            continue
    ...
    if is_imperial:
        ...
        for name, yomi in IMPERIAL_SOLO_NAMES.items():
            pattern = rf'(?<![一-龥ぁ-んァ-ヶA-Za-z]){re.escape(name)}(?=[のはがにをもへと、。\s]|$)'
            ...
            with urllib.request.urlopen(req_ext, timeout=_WIKI_TIMEOUT, context=ctx) as r_ext:
                ...
                for pid, pdata in data_ext.get("query", {}).get("pages", {}).items():
                    ...
                    if re.search(r'[\(（][^\)）]*?(?:\d{3,4}年(?:\d{1,2}月\d{1,2}日)?\s*[-〜–—]|[-〜–—]\s*\d{3,4}年|生年不詳|没年不詳|年生|年没)[^\)）]*?[\)）]', extract[:300]):
                        is_person = True
                    # シグナル2: 人物の職業・身分・属性が明記されている（キャッチコピー等の引用符内を除く）
...
def apply_legal_and_general_rules(text: str) -> str:
    ...
    # 4. 「水上」（すいじょう vs みずかみ vs みなかみ）の形態素誤読防止
    # A. 地名（水上温泉、水上駅、水上町、水上市、水上IC等）➔ みなかみ
    t = re.sub(r'水上(?=(?:温泉|駅|町|市|IC|インター))', 'みなかみ', t)
    # B. 水上（すいじょう）の乗り物・活動・交通・スポーツ・普通名詞
    # （OpenJTalkが「水上バイク」「水上スキー」「水上バス」「水上交通」等を人名「ミズカミ」と誤判定するのを根絶）
    t = re.sub(
        r'水上(?=(?:バイク|スキー|バス|交通|タクシー|飛行機|コテージ|安全|レジャー|活動|警察|輸送|パレード|オートバイ|競技|スポーツ|ドローン|太陽光|スキー場|事故|遭難|救助|捜索|パトロール|航行|運行|運航|施設|遊覧|遊具|散策|歩行|移動|訓練|演習|都市|ルート|生活|住宅|拠点|機|戦|艦|置換|生活者|ステージ|花火|[をにでへとからも]))',
        'すいじょう',
        t
    )
