...
def save_to_pronunciation_memory(surface: str, reading: str, sample_sentence: str = "", note: str = "", category: str = "web_auto_learned") -> bool:
    """判明した読み方を pronunciation_memory.json に永続追加・保存"""
    if not surface or not reading or surface == reading:
        return False
    try:
        mem_data = {"version": "1.0", "records": []}
        ...
        with open(PRONUNCIATION_MEMORY_PATH, "w", encoding="utf-8") as f:
            json.dump(mem_data, f, ensure_ascii=False, indent=2)
        print(f"🧠 [自動記憶] 新しい読み方を台帳に保存しました: '{surface}' ➔ '{reading}'", flush=True)
        return True
...
def resolve_unknown_reading_online(term: str, auto_save: bool = True) -> Optional[str]:
    """
    未知語の読み方を Web（Wikipedia ➔ DuckDuckGo）で自動解決。
    成功した場合は自動で pronunciation_memory.json に保存して永続学習。
    """
    ...
    print(f"ℹ️ [Web読み方未検出] '{term}' の読み方はWebから特定できませんでした。", flush=True)
    return None
...
def extract_candidate_terms_from_text(text: str) -> List[str]:
    ...
    # 1. 漢字＋数字（乃木坂46、櫻坂46等）
    for m in re.finditer(r'([\u4e00-\u9fa5]{2,6}\d{1,4})', text):
        candidates.append(m.group(1))

    # 2. カタカナ＋数字
    for m in re.finditer(r'([\u30a1-\u30f6ー]{2,8}\d{1,4})', text):
        candidates.append(m.group(1))

    # 3. 英字＋数字（AKB48, F15, iPhone16等）
    for m in re.finditer(r'(?<![A-Za-z0-9])([A-Za-z]{1,8}\d{1,4})(?![A-Za-z0-9])', text):
        candidates.append(m.group(1))

    # 4. 敬称・肩書つきの固有名詞候補（2〜4文字漢字）
