...
- **配信コアタイム中の開発環境テスト・高負荷処理の厳格自粛（CPU/GPU・Ollama保護）**:
  - 本番配信中はローカルLLM（Ollama `qwen2.5:7b`）やVOICEVOXがリアルタイムに高負荷で推論を行っている。
  - 同一Macのマシンリソース（CPU/GPU・メモリ帯域）を奪い合うことで、本番のAI生成タイムアウトや無音事故を誘発するリスクがあるため、**配信コアタイム中の開発環境での単体テスト実行、重いバッチスクリプト実行、サーバー再起動・負荷検証を厳禁** とする。
  - 配信時間帯に行う開発作業は「コードの静的編集・設計確認」にとどめ、実際のテスト実行・結合検証は **必ず配信時間外（10:00〜18:00 または深夜）** に実施すること。
- **本番配信とテスト環境の完全分離**:
  - **本番配信**: macOS ネイティブアプリ（独立ウィンドウ・OBS安定キャプチャ）で実施。
  - **動作検証・テスト**: Safari 等のブラウザ（`http://localhost:8444/...`）からアクセスして検証・デバッグを行うこと。
