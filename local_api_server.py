...
以下のニュース記事（タイトルと概要）の内容を、元記事の事実に基づいて分かりやすく要約し、リスナーに伝えてください。

【構成の厳格ルール（全体で3〜4文のコンパクト構成）】:
・1文目: {transition}
・2〜3文目: 【ニュースタイトル】と【概要】の内容を、確定した事実のみを分かりやすく要約して伝える（概要にない推測や追加情報は絶対に含めない）。
・最後の1文: ニュースに対する率直で親しみやすい一言感想（「〜ですね」「〜期待が高まりますね！」など1文のみで簡潔に締める）。

【話し方・トーンの重要ルール】:
0. **【完全日本語厳守・中国語絶対禁止】必ず100%標準語の自然な日本語のみで出力してください。中国語文字（听、这、那、很、吃惊、吗、的、了等）は1文字も混入させないこと。**
1. **長文・引き伸ばしの禁止**: 1文は20〜35文字程度。ダラダラと長く語らず、要点のみを3〜4文でテンポよく話してください。
2. **語尾の使いすぎ禁止**: 毎文無理にキャラクター語尾をつけず、通常の丁寧語（〜です、〜ですね）を基本とし、要所にのみ自然に添えてください。
3. **前置き挨拶の重複・末尾配置の絶対禁止**: {transition}は冒頭に1度だけ発話し、2文目以降や末尾に「次のニュース」「続いてのニュース」等を入れないこと。
...
6. **記事に書かれていない事実の推測・でっち上げ（ハルシネーション）の絶対厳禁**:
   【概要】に書かれていない未記載の人名、順位、設定を勝手に想像して話すことは厳禁です。概要に記載されている事実のみを正確に要約してください。
