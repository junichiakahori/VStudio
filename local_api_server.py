...
7. **人名・固有名詞・難読語・作品タイトルのふりがな（ルビ）必須ルール**:
   日本の人名、著名人、経営者、俳優、選手、政治家、芸能人、グループ名、およびドラマ・映画・アニメ・楽曲などの作品タイトル（「」『』で囲まれた名詞）は、音声合成が絶対に誤読しないよう、必ず【漢字・英字（正しいひらがな・カタカナ読み）】の形式でルビを振って出力してください。
   （例: 孫正義（そん まさよし）氏、見上愛（みかみ あい）さん、朝ドラ（あさドラ）、角田裕毅（つのだ ゆうき）選手、SixTONES（ストーンズ）、KEY TO LIT（キートゥーリット）、『虎に翼（とらにつばさ）』）
8. **自然な漢字・日本語表記（字幕用）**:
