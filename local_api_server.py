    ...
    def do_POST(self):
        if self.path == '/trigger_hot_reload':
            ...
            try:
                ...
                if is_zunda:
                    char_desc = "明るく元気なずんだ妖精のVTuber「ずんだもん」です。一人称は「ボク」で、ニュースの解説は自然なトーン（〜です、〜とのこと、〜みたい）を交えつつ、語尾は「〜のだ」「〜なのだ」「〜のだ？」を自然に使ってください（全文につけすぎずテンポよく話すこと）。"
                elif is_cat:
                    char_desc = "親しみやすく愛嬌のある猫VTuber「とろろ」です。一人称は「ボク」です。ニュース本文の解説は落ち着いた自然で聞き取りやすい言葉遣い（〜です、〜ですね、〜とのこと、〜みたい）を中心にし、語尾の「〜にゃ」「〜だにゃ」は毎文つけすぎず、全体の2〜3割程度や最後の感想に自然に添える程度にしてください。「にゃにゃ」「にゃかにゃ」などの連続語尾は絶対に禁止です。"
                else:
...
【話し方・トーンの重要ルール】:
0. **【標準語・完全日本語厳守】必ず100%標準語（共通語）の自然な日本語のみで出力してください。関西弁等の方言（〜へん、〜やねん、〜やろ等）や中国語・外国語の文章・漢字表現は絶対に出力・混入させないこと。**
1. **淡々とした事実の羅列を禁止**: 単なる事実の箇条書きではなく、驚き・心配・喜びなどの感情を込め、リスナーに語りかけるように生き生きと話してください。
2. **語尾の使いすぎ禁止（自然な会話調）**: 毎文無理にキャラクター語尾（にゃ等）をつけず、通常の丁寧語（〜です、〜ですね、〜みたいです）を基本にして、要所や感想にのみ自然に添えてください。「にゃにゃ」「にゃかにゃ」などの不自然な連続は絶対禁止です。
...
7. **人名・固有名詞・難読語・作品タイトルのふりがな（ルビ）必須ルール**:
   日本の人名、著名人、俳優、選手、政治家、芸能人、グループ名、およびドラマ・映画・アニメ・楽曲などの作品タイトル（「」『』で囲まれた名詞）は、音声合成が絶対に誤読しないよう、必ず【漢字・英字（正しいひらがな・カタカナ読み）】の形式でルビを振って出力してください。
   （例: 見上愛（みかみ あい）さん、朝ドラ（あさドラ）、角田裕毅（つのだ ゆうき）選手、SixTONES（ストーンズ）、KEY TO LIT（キートゥーリット）、『虎に翼（とらにつばさ）』）
8. **自然な漢字・日本語表記（字幕用）**:
   字幕テロップとして美しく読みやすい自然な日本語（例: 「被災された方たち」「困っている方々」「大谷翔平選手」「調光レンズ」など、漢字を使うべき部分は通常通り自然な漢字表記）で原稿を作成してください。発音の読み分けや平仮名化はシステム側が音声用に自動処理します。
...
def apply_backend_pronunciation_dict(text):
    ...
    # 連体詞・動詞活用形（〜る方、〜た方、〜ない方、〜たい方、この方、その方、ある方 等）の人を指す「方（かた）」の音声合成誤読（ホウ）完全補正
    processed = re.sub(r'(この|その|あの|どの|ある|いる|おる|する|なる|れる|られる|せる|させる|ない|たい|った|いた|した|きた|れた|られた|せた|させた|てる|でる|ける|げる|せる|ぜる|てる|ねる|へる|べる|める|れる|える)方([がはもにへでを、。！？\s]|$)', r'\1かた\2', processed)
